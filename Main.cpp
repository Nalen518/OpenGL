...
int main() {
    ...
    std::cout << "  [F]       = cycle shader mode (Phong/Attenuation/Rim/Emission)\n";
    ...
    // Rubber — dark matte, almost no specular, no emission
    Material matRubber = {
        {0.05f, 0.05f, 0.05f},
        {0.50f, 0.50f, 0.50f},
        {0.07f, 0.07f, 0.07f},
    ...
    // Chrome — mirror-like, extreme specular
    Material matChrome = {
        {0.25f, 0.25f, 0.25f},
        {0.40f, 0.40f, 0.40f},
        {0.97f, 0.97f, 0.97f},
    ...
    // Jade — green tones, soft specular, subtle emission
    Material matJade = {
        {0.14f, 0.22f, 0.16f},
        {0.54f, 0.89f, 0.63f},
        {0.32f, 0.32f, 0.32f},
    ...
    // Obsidian — very dark, slight purple specular
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        if (gConfig == 1) {
            // Config 1: Standard perspective — front view 60 FOV
            camPos = glm::vec3(0.f, 1.f, 12.f);
            ...
            // Config 2: Dramatic low angle — looking up at model
            camPos = glm::vec3(0.f, -3.f, 8.f);
            view = glm::lookAt(camPos,
                glm::vec3(0.f, 1.f, 0.f),
                glm::vec3(0.f, 1.f, 0.f));
            projection = glm::perspective(
            ...
            // Config 3: Orthographic side view — no perspective distortion
            camPos = glm::vec3(10.f, 0.f, 0.f);
            view = glm::lookAt(camPos,
                glm::vec3(0.f, 0.f, 0.f),
                glm::vec3(0.f, 1.f, 0.f));
            float h = 4.f;
        ...
        // ================================================
        //  Draw 6 objects — 2 rows of 3, different materials
        // ================================================

        // Row 1 — Gold, Plastic, Rubber (front row)
        // Object 1 — Gold, rotating slowly
        {
            glm::mat4 model = glm::mat4(1.f);
            model = glm::translate(model, glm::vec3(-4.f, -1.f, 0.f));
            model = glm::rotate(model, time * 0.5f, glm::vec3(0.f, 1.f, 0.f));
        ...
        // Object 2 — Cyan Plastic, slight angle
        {
            glm::mat4 model = glm::mat4(1.f);
        ...
        // Object 3 — Rubber, slightly larger
        {
            glm::mat4 model = glm::mat4(1.f);
            model = glm::translate(model, glm::vec3(4.f, -1.f, 0.f));
            model = glm::scale(model, glm::vec3(0.036f));
            glm::mat3 normalMatrix = glm::mat3(glm::transpose(glm::inverse(model)));
            phong.setMat4("model", model);
            phong.setMat3("normalMatrix", normalMatrix);
            applyMaterial(phong, matRubber);
            ourModel.Draw(phong);
        }

        // Row 2 — Chrome, Jade, Obsidian (back row)
        // Object 4 — Chrome, counter-rotating
        {
            glm::mat4 model = glm::mat4(1.f);
            model = glm::translate(model, glm::vec3(-4.f, -1.f, -5.f));
        ...
        // Object 5 — Jade, tilted
        {
            glm::mat4 model = glm::mat4(1.f);
            model = glm::translate(model, glm::vec3(0.f, -1.f, -5.f));
            model = glm::rotate(model, glm::radians(15.f), glm::vec3(1.f, 0.f, 0.f));
            model = glm::rotate(model, time * 0.3f, glm::vec3(0.f, 1.f, 0.f));
            model = glm::scale(model, glm::vec3(0.03f));
            glm::mat3 normalMatrix = glm::mat3(glm::transpose(glm::inverse(model)));
            phong.setMat4("model", model);
            phong.setMat3("normalMatrix", normalMatrix);
            applyMaterial(phong, matJade);
            ourModel.Draw(phong);
        }
        // Object 6 — Obsidian, static
        {
            glm::mat4 model = glm::mat4(1.f);
            model = glm::translate(model, glm::vec3(4.f, -1.f, -5.f));
            model = glm::scale(model, glm::vec3(0.03f));
            ...
            phong.setMat4("model", model);
            phong.setMat3("normalMatrix", normalMatrix);
            applyMaterial(phong, matObsidian);
            ourModel.Draw(phong);
        }
